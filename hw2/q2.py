...
def holdout(model, xFeat, y, testSize):
    ...
    yTestPredict = model.predict_proba(X_test)[:,1]
    yTrainPredict = model.predict_proba(X_train)[:,1]
    trainAuc = roc_auc_score(y_train, yTrainPredict)
    testAuc = roc_auc_score(y_test, yTestPredict)
    end = time.time()
...
def kfold_cv(model, xFeat, y, k):
    ...
    for train_index, test_index in kf.split(xFeat):
        ...
        yTrainPredict = model.predict_proba(X_train)[:,1]
        yTestPredict = model.predict_proba(X_test)[:,1]
        totalTrainScore += roc_auc_score(y_train, yTrainPredict)
        totalTestScore += roc_auc_score(y_test, yTestPredict)

    trainAuc = totalTrainScore/k
    testAuc = totalTestScore/k
    end = time.time()
    timeElapsed = end - start
    return trainAuc, testAuc, timeElapsed
...
def mc_cv(model, xFeat, y, testSize, s):
    ...
    monte_carlo = ShuffleSplit(n_splits=s, test_size = testSize)
    totalTrainScore = 0.0
    totalTestScore = 0.0
    
    for train_index, test_index in monte_carlo.split(xFeat):
        X_train, X_test = xFeat.iloc[train_index,:], xFeat.iloc[test_index,:]
        y_train, y_test = y.iloc[train_index], y.iloc[test_index]
        model.fit(X_train, y_train)
        yTrainPredict = model.predict_proba(X_train)[:,1]
    ...
    trainAuc = totalTrainScore/s
    testAuc = totalTestScore/s
