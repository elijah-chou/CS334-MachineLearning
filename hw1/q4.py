...
def standard_scale(xTrain, xTest):
    ...
    xTrain_copy = scaler.fit_transform(xTrain_copy)
    xTest_copy = scaler.transform(xTest_copy)
    xTrainFinal = pd.DataFrame(xTrain_copy)
    xTestFinal = pd.DataFrame(xTest_copy)
...
def add_irr_feature(xTrain, xTest):
    ...
    xTrain_copy = xTrain.copy()
    xTest_copy = xTest.copy()
    train_size = xTrain_copy.shape[0]
    test_size = xTest_copy.shape[0]

    gauss_feature1 = np.random.normal(mu, sigma, train_size)
    gauss_feature2 = np.random.normal(mu, sigma, train_size)
    gauss_feature3 = np.random.normal(mu, sigma, test_size)
    gauss_feature4 = np.random.normal(mu, sigma, test_size)

    xTrain_copy['Noisy Feature 1'] = gauss_feature1.tolist()
    xTrain_copy['Noisy Feature 2'] = gauss_feature2.tolist()
    xTest_copy['Noisy Feature 1'] = gauss_feature3.tolist()
    xTest_copy['Noisy Feature 2'] = gauss_feature4.tolist()
...
def main():
    ...
    xTrainIrr, xTestIrr = add_irr_feature(xTrain, xTest)
    acc4 = knn_train_test(args.k, xTrainIrr, yTrain, xTestIrr, yTest)
